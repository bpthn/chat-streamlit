...
def run(user_input):
    ...
    #     st.markdown(age)
    # print('age')
    # st.session_state.diagnosis.append(age)
...
def compliants(age, sex):
    
    args = parse_args()
    auth_string = get_auth_string(args.auth)
    case_id = new_case_id()  
    # print('a')
    # age, sex = conversation.read_age_sex(age)
    # print(f"Ok, {age} year old {sex}.")
    # age = {'value':  age, 'unit': 'year'}
    # print('age')
...
def test(user_input):
    ...
    if st.session_state.diagnosis == False:
        run(user_input)
        print('no age found')
    else:
        print('alrdy age')
